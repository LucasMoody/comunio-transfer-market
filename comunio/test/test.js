...
describe('#getMarketForUser(userName)', function() {
  this.timeout(20000);
  it('should return an array which length is greater than 0', function() {
    return comunio.getMarketForUserName('Luque88').should.eventually.have.length.above(0);
  });
  it('should return an array of players where all elements have the properties id, clubid, position, name, points, quote, status, recommendedprice, ownerid, placed', function() {
    return comunio.getMarketForUserName('Luque88').should.eventually.all.contain.keys('id', 'clubid', 'position', 'name', 'points', 'quote', 'status', 'recommendedprice', 'ownerid', 'placed');
  });
  it('should return an array which length is greater than 0 for comunio id 1605015', function() {
    return comunio.getMarketForUserName('Luque88').should.eventually.have.length.above(0);
  });
