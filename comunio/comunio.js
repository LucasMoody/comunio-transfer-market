...
function getMarketForUserName(user) {
  return getCommunityByUser('Luque88').
  then(community => {
    return getCommunityMarket(community.id)
  });
}

function getCommunityNews(communityId) {
  var args = {communityid : communityId, days: '2'};
  return Q.nfcall(soap.createClient, url)
    .then(function(client) {
      return Q.nfcall(client.getcomputernews, args);
    })
    .then(function(news) {
      if(news[0].return) {
        if(news[0].return.item instanceof Array) {
          return news[0].return.item.map(item => item.message.$value)
        } else {
          return [news[0].return.item.message.$value];
        }
      }
    })
    .then(news => {
      return news.map(singleNews => cheerio.load(singleNews).text())
    });
}
...
module.exports = {
	...
	getUserId: getUserId,
  getMarketForUserName: getMarketForUserName,
